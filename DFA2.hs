...
data Re a f x where
    ...
    SymX :: Maybe [a] -> Re a f (f a)
    -- Alternative, |
    AltX :: Re a f x -> Re a f y -> Re a f (Alt x y)
    -- Intersection
    CutX :: Re a f x -> Re a f y -> Re a f (Cut x y)
    -- Sequencing
    SeqX :: Re a f x -> Re a f y -> Re a f (Seq x y)
    -- Repetition, Kleene Star *
    RepX :: Re a f x -> Re a f (Rep x)
    -- Plus :: Re a x -> Re a (NonEmptyList x)
    -- Complement
    NotX :: Re a f x -> Re a f (Not x)
    -- Match empty string
    EpsX :: x -> Re a f (Eps x)
    -- Match no string
    NilX :: Re a f Nil 
    ...
    -- This might not work!
    FMapX :: (x -> y) -> Re a f x -> Re a f (FMap x y)
    deriving (Typeable)

data BoolBefore a = BoolBefore Bool
type ReBBefore a x = Re a BoolBefore x

data BoolAfter a = BoolAfter Bool
